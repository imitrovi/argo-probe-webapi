...
def CheckResults(arguments, profilesjson):
    ...
    while (count < allReports):
        ...
        # check for connection request
            ...
            ProbeDescription['Connection' +
                             reportName] = 'CRITICAL - Reports cannot connect to (https://' + arguments.hostname + pathToUse+'/' + reportName+'/'+reportTopologyGroup+')'
            ProbeDescription['Status' +
                             reportName] = 'CRITICAL - Status code error. Cannot connect to (https://' + arguments.hostname + pathToUse+'/' + reportName+'/'+reportTopologyGroup+')'

        # when we have results report
        # check the json for the string we need
        if (arguments.rtype == 'ar'):
            ProbeDescription[reportName] = 'OK - Availability for ' + \
                reportName + ' is OK'
            try:
        ...
        if (arguments.rtype == 'status'):
            ProbeDescription[reportName] = 'OK - status for ' + \
                reportName + ' is OK'
            try:
...
def main():
    ...
    parser.add_argument('--tenant', dest='tenant', required=True,
                        type=str, default='EGI', help='tenant to check. Default EGI')
    parser.add_argument('--rtype', dest='rtype', required=True,
                        type=str, default='ar', help='status or ar (default ar)')
    parser.add_argument('--token', dest='token', required=True,
                        type=str, default='test_token', help='authentication token')
    parser.add_argument('--unused-reports', dest='unusedreports', required=False,
                        type=str, nargs='+', help='Add unused reports from API')
