...
## ovo su fake report-ovi za funkciju CheckResults
...
class ArgoProbeWebApiTests(unittest.TestCase):
    def setUp(self) -> None: 
        arguments = {"tenant":"mock-tenanat", "token":"1234", "hostname":"mock_hostname", "timeout": 5, "rtype": 'status', "day":1, "unusedreports": 0, "debug": ""} 

        self.arguments = SimpleNamespace(**arguments)      


    @patch("argo_probe_webapi.web_api.requests.get")
    def test_api_call_url_ar(self, mock_request):
        mock_request.side_effect = mock_web_api

        apiCallUrl = createAPICallUrl(self.arguments)

        mock_request.assert_called_once_with("https://mock_hostname/api/v2/reports", headers={'Accept': 'application/json', 'x-api-key': '1234'}, timeout=5) 
    ...
    def test_fail_api_call_url(self, mock_request, mock_print):
        ...
        mock_print.assert_has_calls([(call("CRITICAL - API cannot connect to https://mock_hostname/api/v2/reports"))]) 



    def test_convert_date_start(self):
        expected = convert_date(2020, 4, 22, "start")
        actual = "2020-04-22T00:00:00Z"
        self.assertEqual(expected, actual)
    
    def test_convert_date_end(self):
    ...
    @patch("builtins.print")      
    def test_utils_NAGIOS_0(self, mock_print):
      ...
      calls = [
        call(f'Ok - All {self.arguments.rtype} reports for tenant mock-tenanat return results'),
    ...
    @patch('builtins.print')
    def test_utils_NAGIOS_1(self, mock_print):
      ...
      mock_print.assert_has_calls(calls)
      self.assertEqual(e.exception.__str__(), "1")



    @patch('builtins.print')
    def test_utils_NAGIOS_2(self, mock_print):
      ...
      calls = [
        call(f'CRITICAL - Problem with {self.arguments.rtype} reports for tenant {self.arguments.tenant} return results'),
